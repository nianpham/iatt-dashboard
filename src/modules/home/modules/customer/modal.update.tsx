...
export function ModalUpdateCustomer({ data }: { data: any }) {
  ...
  const [mainPreview, setMainPreview] = useState(data.avatar || "");
  ...
  const handleSubmit = async () => {
    ...
    const formattedData = {
      ...
      avatar: uploadMainImage[0].secure_url || IMAGES.DEFAULT_AVT,
  ...
  const handleDelete = async () => {
    ...
    await AccountService.deleteAccount(data?._id);
